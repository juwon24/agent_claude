...
- Using the PGI (Petrophysically and Geologically guided Inversion) framework
- Including full petrophysical information in geophysical inversions
- Multi-physics inversion with dynamic Gaussian mixture model prior
- Creating and using petrophysical GMM with known rock unit properties
- Recovering quasi-geology models from geophysical data
...
Keywords: PGI, joint inversion, gravity, magnetics, petrophysical inversion, GMM
"""
...
print(f"  True geology model loaded")
print(f"  Rock units: Background (0), PK (1), VK (2)")
print(f"  PK properties: -0.8 g/cc density, 5e-3 SI susceptibility")
print(f"  VK properties: -0.2 g/cc density, 2e-2 SI susceptibility")
print()
...
ax[3].set_aspect(10)
plt.savefig("pgi_full_info_true_geology.png", dpi=150, bbox_inches="tight")
print("✓ True geology model plot saved as 'pgi_full_info_true_geology.png'")
plt.close()
...
plt.savefig("pgi_full_info_data.png", dpi=150, bbox_inches="tight")
print("✓ Data plot saved as 'pgi_full_info_data.png'")
plt.close()
...
# ============================================================================
# Step 7: Create Petrophysical GMM with Full Information
# ============================================================================

print("=" * 80)
print("Step 7: Creating Petrophysical GMM with Full Information")
print("=" * 80)
...
print(f"  Background: density=0.0 g/cc, susceptibility=0.0 SI")
print(f"  PK unit: density=-0.8 g/cc, susceptibility=0.005 SI")
print(f"  HK unit: density=-0.2 g/cc, susceptibility=0.02 SI")
print()
...
ax[2].set_xlim([0, 100])
plt.savefig("pgi_full_info_gmm_initial.png", dpi=150, bbox_inches="tight")
print("✓ Initial GMM plot saved as 'pgi_full_info_gmm_initial.png'")
plt.close()
...
update_smallness = directives.PGI_UpdateParameters(update_gmm=False)
...
print(f"    - Multi-target misfits")
print(f"    - PGI parameter updates (GMM fixed)")
...
print("=" * 80)
print("Step 10: Running PGI Inversion with Full Petrophysical Information")
print("=" * 80)
...
print(f"  Density model range: [{np.nanmin(density_model):.4f}, {np.nanmax(density_model):.4f}] g/cc")
print(f"  Susceptibility model range: [{np.nanmin(magsus_model):.4f}, {np.nanmax(magsus_model):.4f}] SI")
print(f"  Quasi-geology classifications: {np.unique(quasi_geology_model[~np.isnan(quasi_geology_model)])}")
print()
...
plt.tight_layout()
plt.savefig("pgi_full_info_recovered_models.png", dpi=150, bbox_inches="tight")
print("✓ Recovered models plot saved as 'pgi_full_info_recovered_models.png'")
plt.close()

# Plot the 2D GMM with recovered model
...
plt.savefig("pgi_full_info_gmm_recovered.png", dpi=150, bbox_inches="tight")
print("✓ GMM with recovered model plot saved as 'pgi_full_info_gmm_recovered.png'")
plt.close()
...
print(f"  Recovered density:       [{np.nanmin(density_model):.4f}, {np.nanmax(density_model):.4f}] g/cc")
print(f"  Recovered susceptibility:[{np.nanmin(magsus_model):.4f}, {np.nanmax(magsus_model):.4f}] SI")
print()
print("Generated Files:")
print("-" * 80)
print("  1. pgi_full_info_true_geology.png      - True geology model (3 views)")
print("  2. pgi_full_info_data.png              - Gravity and magnetic data")
print("  3. pgi_full_info_gmm_initial.png       - Initial GMM distribution")
print("  4. pgi_full_info_recovered_models.png  - Recovered models (geology/density/susceptibility)")
print("  5. pgi_full_info_gmm_recovered.png     - GMM with recovered model overlay")
print()
print("Key Features:")
print("-" * 80)
print("  - Joint inversion of gravity and magnetic data")
print("  - PGI framework with FULL petrophysical information")
print("  - GMM with known rock unit properties (means and covariances)")
print("  - Quasi-geology model recovered from geophysical data")
print("  - Structural similarity enforced through petrophysical coupling")
print()
print("Note: This inversion used complete petrophysical information about the")
print("      rock units. Compare with the 'no info' version to see the difference!")
print("=" * 80)
